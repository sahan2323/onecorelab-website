...
// Both assets are served from our own origin. They used to come from
// jsdelivr and raw.githubusercontent.com, which meant every headline on the
// site depended on two third-party requests succeeding — slow or filtered
// mobile networks turned that into visibly missing text (and, in the intro,
// a hang). Self-hosted they are same-origin, cached with the deploy, and
// work offline.
const OPENTYPE_CDN = "/vendor/opentype.min.js";
...
export interface HandwritingTextProps {
  ...
  height?: string;
  className?: string;
  /** Fired once the fill has finished — lets a parent sequence the next beat. */
  onDrawn?: () => void;
}
...
export function HandwritingText({
  ...
  // Let a parent know when the whole word has finished writing + inking.
  useEffect(() => {
    if (!drawn || !onDrawnRef.current) return undefined;
    const ms = (delay + duration + 0.45) * 1000;
    const id = setTimeout(() => onDrawnRef.current?.(), ms);
    return () => clearTimeout(id);
  }, [drawn, delay, duration]);
